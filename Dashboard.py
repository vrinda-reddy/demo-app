...
def safe_calculate_performance(index_data, start_date, end_date):
    ...
    try:
        ...
        # Ensure we return scalar values, not Series
        if hasattr(pct_change, 'iloc'):
            pct_change = pct_change.iloc[0] if len(pct_change) > 0 else 0.0
        if hasattr(volatility, 'iloc'):
            volatility = volatility.iloc[0] if len(volatility) > 0 else 0.0
            
        return float(pct_change), float(volatility)
