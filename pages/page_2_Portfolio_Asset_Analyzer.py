...
def get_benchmark_data(ticker_to_download, start_date, end_date):
    """Fetch and process benchmark data - cached for 1 hour"""
    from datetime import timedelta
    ...
    try:
        ...
        # ⬇️ ADD THIS LINE (persists raw download)
        ...
        if not data_all.empty:
            ...
            # ⬇️ ADD THIS LINE (native-currency benchmark series)
            st.session_state.benchmark_prices = price_series

            # Get currency info
            benchmark_info = yf.Ticker(ticker_to_download).info
...
def get_earliest_available_data(ticker):
    try:
        ...
        hist = ticker_obj.history(period="max")  # Get maximum available history
        earliest_date = hist.index[0]  # The first date in the historical data
...
def is_valid_ticker_symbol(ticker):
    ...
    try:
        ticker_obj = yf.Ticker(ticker)
        # Just get basic info - much faster than downloading price data
...
def safe_ticker_search(search_input):
    ...
    if len(search_input) <= 6 and search_input.replace('.', '').replace('-', '').isalpha():
        ...
        if is_valid_ticker_symbol(search_input.upper()):
            try:
                ticker_obj = yf.Ticker(search_input.upper())
                info = ticker_obj.info
    ...
    if st.button("Add Direct Ticker") and manual_ticker:
        ...
        if ticker_to_add not in existing:
            if is_valid_ticker_symbol(ticker_to_add):
                try:
                    ticker_obj = yf.Ticker(ticker_to_add)
                    info = ticker_obj.info
